...
describe('Upload Berita Acara RCM — Skenario Positif', () => {
  ...
  it('Berhasil isi form, upload file, save draft, submit, dan verifikasi detail', () => {
    ...
    UploadBeritaAcaraRcmFormPage.fillLaporanKeuanganTerakhir(
      testData.tanggalLaporanTerakhir,
      testData.statusLaporanTerakhir
    );

    // ---- Langkah 9: Laporan Keuangan Tahun Sebelumnya ----
    UploadBeritaAcaraRcmFormPage.fillLaporanKeuanganSebelumnya(
      testData.tanggalLaporanSebelumnya,
      testData.statusLaporanSebelumnya
    );

    // ---- Langkah 10–11: Profile Bisnis & Financial ----
    UploadBeritaAcaraRcmFormPage.selectDropdownByLabel('Profile Bisnis', testData.profileBisnis);
    UploadBeritaAcaraRcmFormPage.selectDropdownByLabel('Profile Financial', testData.profileFinancial);
    ...
    UploadBeritaAcaraRcmFormPage.selectDropdownByLabel('Stand Alone Rating', testData.standAloneRating);
    UploadBeritaAcaraRcmFormPage.selectDropdownByLabel('Stand Alone Outlook', testData.standAloneOutlook);
    UploadBeritaAcaraRcmFormPage.selectDropdownByLabel('Final Rating', testData.finalRating);
    UploadBeritaAcaraRcmFormPage.selectDropdownByLabel('Final Outlook', testData.finalOutlook);

    // ---- Langkah 17–20: Status Lainnya — text input ----
    UploadBeritaAcaraRcmFormPage.fillParentName(testData.parentName);
    UploadBeritaAcaraRcmFormPage.selectDropdownByLabel('Parent Action', testData.parentAction);
    UploadBeritaAcaraRcmFormPage.fillParentComment(testData.parentComment);
    UploadBeritaAcaraRcmFormPage.fillGuarantor(testData.guarantor);

    // ---- Langkah 21–23: Upload file via popup Tambah ----
    UploadBeritaAcaraRcmFormPage.clickTambah();
    UploadBeritaAcaraRcmFormPage.uploadFileInPopup(testData.uploadFile);
